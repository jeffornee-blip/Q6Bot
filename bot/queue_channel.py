...
import bot
from bot.stats.rating import FlatRating, Glicko2Rating, TrueSkillRating, Quidditch6v6Rating
...
class QueueChannel:
	...
	rating_names = {
		...
		'Quidditch6v6': Quidditch6v6Rating
	}
	...
	async def queue_started(self, ctx, members, message=None):
		...
		for m in filter(lambda m: m.id in bot.allow_offline, members):
			bot.allow_offline.pop(m.id, None)
		if message:
